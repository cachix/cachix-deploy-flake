{
  ...
  outputs =
    { self, nixpkgs, darwin, home-manager, nixos-anywhere, disko, ... }: {
      ...
      in forAllSystems (system:
        ...
        in {
          bootstrapHetzner = pkgs.writeScriptBin "bootstrap-hetzner" ''
            ...
            }/bin/nixos-anywhere "root@$IP" --flake ".#$agent"
      ...
      lib = pkgs: {
        ...
        bootstrapNixOS = { system, hostname, grubDevices ? [ ], diskoDevices ?
            import ./disko-raid1-uefi.nix {
              disks = grubDevices;
              inherit (pkgs) lib;
            }, sshPubKey, imports ? [ ] }:
          let
            module = { pkgs, ... }: {
              imports = [ disko.nixosModules.disko diskoDevices ] ++ imports;
              ...
              config = {
                ...
                boot.initrd.availableKernelModules =
                  [ "nvme" "md_mod" "raid1" "xhci_pci" "ahci" ];
